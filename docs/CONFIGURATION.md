...
This guide covers the optional features and their configuration in formexa.
...
> **Note:** Ollama models are discovered dynamically at runtime when an Ollama server is available. Only models that expose the `tools` capability will appear in formexa, so you no longer need to keep placeholder Ollama entries in `models.json`.
...
When this variable is set, formexa will automatically discover Ollama models that advertise the `tools` capability. Models without this capability are ignored, and no static configuration in `models.json` is required unless you need to override specific settings.
